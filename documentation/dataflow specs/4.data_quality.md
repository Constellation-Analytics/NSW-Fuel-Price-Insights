...
2. Call the `public.data_quality_check()` stored procedure.
...
- Database connection wrapped in try/except
- Procedure execution errors logged
- Critical failures propagate to orchestrator
- Workflow logs include timestamps for execution start and completion

---

## 10. Cloud Deployment Considerations

- Replace any local connection strings with `DB_CONNECTION_STRING` from environment variables
- Ensure GitHub Actions or cloud runner has appropriate database network access
- Logging integrates with GitHub Actions logs
- Stored procedure must exist and be accessible by workflow user
